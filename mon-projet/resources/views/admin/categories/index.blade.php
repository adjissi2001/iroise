...
@section('content')
    ...
    <div class="mb-10 bg-white rounded-lg border border-gray-200 p-5">
        <h3 class="font-semibold text-lg text-gray-800 mb-4">Ajouter une catégorie</h3>

        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4 max-w-3xl">
            @csrf
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse(($categories ?? collect()) as $cat)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-sm text-gray-700">{{ $cat->id_categorie ?? '-' }}</td>
                            <td class="px-4 py-2 text-sm text-gray-900 font-medium">{{ $cat->nom_categorie ?? '-' }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700">{{ $cat->description ?? '' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-4 text-sm text-gray-500">Aucune catégorie trouvée.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
